...
class FGVC(Dataset):
    def __init__(self, root, name=None, train=True, transform=None, target_transform=None):
        if "cub" in name:
            root = os.path.join(root, "cub2011_200")
        elif "nabird" in name:
            root = os.path.join(root, "nabirds_555")
        elif "flower" in name:
            root = os.path.join(root, "oxfordflowers_102")
        elif "dog" in name:
            root = os.path.join(root, "stanforddogs_120")
        elif "car" in name:
            root = os.path.join(root, "stanfordcars_196")
        ...
        train_split = read_json(os.path.join(root, "split/train.json"))
        val_split = read_json(os.path.join(root, "split/val.json"))
        test_split = read_json(os.path.join(root, "split/test.json"))
