...
class synapse_plasticity(nn.Module):
    ...
    def __init__(self, weight, bias, presynapse: neurotransmitter = None): # type: ignore
        ...
        if self.presynapse:
            self.presynapse_value = self.presynapse.scale.clone()
            if self.presynapse.scale.shape[0] != self.weight_shape[1]:
                self.presynapse_value = self.resize(self.weight_shape[1])
            self.presynapse_value = self.smooth()
            self.LTD_LTP = LTD_LTP(self.presynapse_value.shape[0])
    ...
    def forward(self, x, nt_sp_both = 'both'):
        if nt_sp_both == 'nt':
            ...
            if self.presynapse_value is not None:
                scale = self.LTD_LTP(self.presynapse_value)
                weight *= scale
    ...
    def resize(self, new_size):
        ...
        '''
        scale_np = self.presynapse_value.reshape(1, 1, -1)  # Reshape to (N, C, L) for interpolation
        scale_resized = F.interpolate(scale_np, size=new_size, mode='linear', align_corners=True)
        return scale_resized.view(-1)  # Flatten back to 1D
    ...
    def smooth(self, window_size=3):
        ...
        scale_np = self.presynapse_value.detach().numpy()
